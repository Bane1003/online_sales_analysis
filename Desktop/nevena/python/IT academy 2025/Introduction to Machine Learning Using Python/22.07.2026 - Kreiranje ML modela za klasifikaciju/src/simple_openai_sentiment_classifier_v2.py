...
# Učitavanje podataka
df = pd.read_csv("data/reviews_labeled_cleaned_sample.csv")

total_review=len(df)
predicted_sentiments=[]

#pruzimanje kontrole nad iteracijom
#ispis da znamo dokle smo stigli
#start=1, da krene od prve recenzije
for i, j in enumerate(df['review'], start=1):
      print(f"{i}/{total_review}")
      predicted_sentiment=classify_sentiment(j)
